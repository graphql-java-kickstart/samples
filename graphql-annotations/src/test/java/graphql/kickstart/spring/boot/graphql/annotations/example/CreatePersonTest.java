...
public class CreatePersonTest {
  ...
  void testCreatePerson(final CreatePerson input, final Person expected) throws IOException {
    // GIVEN
    ...
    // WHEN - THEN
    graphQLTestTemplate
        .perform("create-person.graphql", params)
        .assertThatNoErrorsArePresent()
        .assertThatField("$.data.createPerson.id")
        .asString()
        .isEqualTo(
            personRepository.findAll().stream()
                ...
                .orElseThrow())
        .and()
        .assertThatField("$.data.createPerson")
        .as(Person.class)
